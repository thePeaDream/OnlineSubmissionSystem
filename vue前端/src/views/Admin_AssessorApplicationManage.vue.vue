<template>
  <div style="margin-top:10px;">
    <el-input placeholder="按昵称查找审核员申请者" v-model="searchNickName" @input="searchAssessors"></el-input>
    <el-table :data="assessors" style="margin-top: 20px">
      <el-table-column prop="assessorId" label="审核员申请者ID"></el-table-column>
      <el-table-column prop="nickName" label="昵称"></el-table-column>
      <el-table-column prop="applicationTime" label="申请时间"></el-table-column>
      <el-table-column prop="categoryId" label="擅长类别"></el-table-column>
      <el-table-column prop="phone" label="电话"></el-table-column>
      <el-table-column label="操作">
        <template v-slot="{ row }">
          <el-button type="primary" @click="approveApplication(row)">批准</el-button>
          <el-button type="danger" @click="rejectApplication(row)">拒绝</el-button>
          <div style="margin-top:5px;"></div>
          <el-button type="primary" @click="viewDetail(row)">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog title="审核员申请者详情" :visible.sync="editDialogVisible" style="margin-top:-150px;">
      <el-form :data="editForm">
        <el-form-item label="审核员申请者id">
  ...
  methods: {
    searchAssessors() {
      if (this.searchNickName === '') {
        http.get("/getApplicationAssessor").then
        (response=>{
            this.assessors =response.data.applicationAssessorList;
        }
        )
        .catch(error => {
    ...
    rejectApplication(row){
      ...
      (response=>{
          this.assessors = response.data.applicationAssessorList;
          console.log(response.data.applicationAssessorList)
      }
      )
      .catch(error => {
         console.error('获取审核员申请者列表失败', error);
       });
    },
    approveApplication(row)
    {
      http.post("/setAssessorStatus",row).then
      (response=>{
          this.assessors = response.data.applicationAssessorList;
          console.log(response.data.applicationAssessorList)
      }
